  ...
  return (
    <div className="h-[100dvh] lg:h-screen flex flex-col lg:flex-row bg-background-dark overflow-hidden relative">
      ...
      <section className="flex-1 flex flex-col min-h-0 border-r border-white/5 relative">
        ...
        <header className="p-4 border-b border-white/5 flex items-center justify-between bg-surface-dark/50 backdrop-blur-xl sticky top-0 z-30">
           <div className="flex items-center gap-3">
             <div className="bg-electric-blue/10 p-2.5 rounded-xl text-electric-blue hidden sm:block">
           ...
           <div className="flex items-center gap-3">
             ...
             <button className="p-2 text-white/40 hover:text-white transition-colors bg-white/5 rounded-lg border border-white/5">
               <Maximize2 className="w-4 h-4" />
             </button>
           </div>
        </header>

        <div className="flex-1 overflow-y-auto p-8 lg:p-16 scroll-smooth bg-background-dark custom-scrollbar relative z-10">
           <div className="max-w-3xl mx-auto">
             {isSynthesizing ? (
               ...
               <motion.div 
                 ...
                 prose-code:text-sunset-orange prose-code:bg-white/5 prose-code:px-2 prose-code:py-0.5 prose-code:rounded-md prose-code:font-bold
      ...
      <aside className="w-full lg:w-[400px] h-full flex flex-col bg-surface-dark border-l border-white/10 relative z-20 pb-24 lg:pb-0">
         <header className="p-6 border-b border-white/5 flex items-center justify-between">
           ...
           <button className="text-white/40 hover:text-white" onClick={() => setScreen('dashboard')}>
             <X className="w-5 h-5" />
           </button>
         </header>
         ...
         <div className="p-6 bg-surface-dark space-y-4">
            <div className="grid grid-cols-2 gap-3">
               <button 
                onClick={() => handleSend("Summarize the key concepts of this section in bullet points.")}
                className="py-2.5 px-3 bg-white/5 rounded-xl text-[10px] font-extrabold uppercase tracking-widest text-electric-blue hover:bg-electric-blue/5 transition-all text-center border border-electric-blue/20"
               >
                 Summarize
               </button>
               <button 
                onClick={() => setScreen('quiz')}
                className="py-2.5 px-3 bg-white/5 rounded-xl text-[10px] font-extrabold uppercase tracking-widest text-sunset-orange hover:bg-sunset-orange/5 transition-all text-center border border-sunset-orange/20"
               >
                 Quiz Me
               </button>
            </div>
            ...
            <div className="relative group">
              <textarea 
                ...
                placeholder="Ask about Stochastic Models..."
                className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 pl-5 pr-14 text-sm text-white/80 focus:outline-none focus:ring-1 focus:ring-electric-blue/50 resize-none h-24"
              />
              <button 
                onClick={handleSend}
                className="absolute bottom-4 right-4 p-2.5 bg-electric-blue text-white rounded-xl hover:scale-105 active:scale-95 transition-all shadow-lg shadow-electric-blue/20"
              >
